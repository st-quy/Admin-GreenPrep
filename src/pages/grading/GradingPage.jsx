...
const GradingPage = () => {
  ...
  const extractCommentsFromData = (data, part) => {
    if (!data || !data.data || !data.data.data || !data.data.data.topic || !data.data.data.topic.Parts) {
      return []
    }
    ...
    try {
      ...
      const partData = data.data.data.topic.Parts.find(
        (p) => p.Content && p.Content.toLowerCase().includes(`part ${part}`.toLowerCase()),
      )

      if (partData && partData.Questions) {
        partData.Questions.forEach((question) => {
          if (question.studentAnswer && question.studentAnswer.ID && question.studentAnswer.Comment) {
            comments.push({
  ...
  useEffect(() => {
    if (!isWritingPending && writingData) {
      ...
      for (let part = 1; part <= 4; part++) {
        const partComments = extractCommentsFromData(writingData, part.toString())
  ...
  useEffect(() => {
    if (!isSpeakingPending && speakingData) {
      ...
      for (let part = 1; part <= 4; part++) {
        const partComments = extractCommentsFromData(speakingData, part.toString())
  ...
  const handleCommentChange = (commentData) => {
    const { studentAnswerId, messageContent, isSpeaking, part } = commentData

    if (isSpeaking) {
      // Update speaking comments
      setSpeakingComments((prevComments) => {
        ...
        const existingIndex = prevComments.findIndex(
          (comment) => comment.studentAnswerId === studentAnswerId && comment.part === part,
        )
        if (existingIndex >= 0) {
          // Update existing comment
          const updatedComments = [...prevComments]
          updatedComments[existingIndex] = { studentAnswerId, messageContent, part }
  ...
  const changeParticipant = (participantId) => {
    const newPath = location.pathname.replace(/participant\/[^/]+/, `participant/${participantId}`)
  ...
  const handleNextParticipant = () => {
    const currentIndex = participantsData?.data.data.findIndex((item) => item.ID === participantId)
  ...
  const handlePreviousParticipant = () => {
    const currentIndex = participantsData?.data.data.findIndex((item) => item.ID === participantId)
    const previousIndex = (currentIndex - 1 + participantsData?.data.data.length) % participantsData?.data.data.length
    const previousParticipantId = participantsData?.data.data[previousIndex].ID
    changeParticipant(previousParticipantId)
  }

  const userData = participantsData?.data.data.find((item) => item.ID === participantId)

  if (isWritingPending || isSpeakingPending || isParticipantsPending)
    return <Spin size="large" className="flex justify-center items-center h-60" />
